...
class LateConcatenation_3DCAE(nn.Module):
    def __init__(self):
        super(LateConcatenation_3DCAE, self).__init__()
        # define layers
        ...
        # fc layers for intermediate fusion
        self.fc1 = nn.Linear(8 * 2 * 8 * 8 * 2, 1024)
        self.fc2 = nn.Linear(1024, 8 * 2 * 8 * 8)
    ...
    def forward(self, x1, x2):
        ...
        # *** fusion of encodings
        combined_features = torch.cat((_em3.flatten(1), _em23.flatten(1)), dim=1)
        fused_features = F.relu(self.fc1(combined_features))
        fused_features = F.relu(self.fc2(fused_features))
        fused_features = fused_features.view(_em3.size())

        # *** start of decoder
        _dc1 = F.relu(self.dc1(fused_features))
        ...
        # *** start of decoder
        _dc21 = F.relu(self.dc21(fused_features))
