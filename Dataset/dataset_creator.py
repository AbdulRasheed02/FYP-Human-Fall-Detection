...
def init_videos(img_width, img_height, raw, dset, folder_location):
    ...
    path = "{}\\Dataset\H5PY\\Data_set-{}-imgdim{}x{}.h5".format(
        project_directory, dset, img_width, img_height
    )
    ...
    if len(vid_dir_list_0) == 0 and len(vid_dir_list_1) == 0:
        print(
            "no videos found, make sure video files are placed in Fall-Data folde, terminating..."
        )
        sys.exit()
    ...
    if os.path.isfile(path):
        ...
        if root_path in hf:
            print(
                "video h5py file exists, deleting old group {}, creating new".format(
                    root_path
                )
            )
            del hf[root_path]
...
def flip_windowed_arr(windowed_data):
    ...
    # print(flattened_dim)
    flipped_data_windowed = np.zeros(
        (len(windowed_data), win_len, flattened_dim)
    )  # Array of windows
